...
void	select_sprite(t_data *data, int x, int y)
{
	...
	if (data->map.map[y][x] == '1')
		mlx_put_image_to_window(data->win.mlx, data->win.window, data->sprites.plant, data->win.x, data->win.y);
	...
	if (data->map.map[y][x] == '7')
		mlx_put_image_to_window(data->win.mlx, data->win.window, data->sprites.leaf, data->win.x, data->win.y);
	if (data->map.map[y][x] == 'C')
		mlx_put_image_to_window(data->win.mlx, data->win.window, data->sprites.rupee, data->win.x, data->win.y);
}
...
void	select_sprite_2(t_data *data, int x, int y)
{
    ...
    if (data->map.map[y][x] == '8')
        mlx_put_image_to_window(data->win.mlx, data->win.window, data->sprites.sword, data->win.x, data->win.y);
	...
	if (data->map.map[y][x] == 'E')
		mlx_put_image_to_window(data->win.mlx, data->win.window, data->sprites.exit, data->win.x, data->win.y);
}
...
{	
    if (x == data->player.x && y == data->player.y)
	{
		if (!data->player.steps)
			...
			mlx_put_image_to_window(data->win.mlx, data->win.window, data->player.sprite, data->win.x, data->win.y);
	}
}